// Copyright 2026 Jannik Laugmand Bülow

#ifndef BIBBLEVM_EXECUTOR_SCHEDULER_MESSAGE_H
#define BIBBLEVM_EXECUTOR_SCHEDULER_MESSAGE_H 1

#include "BibbleVM/executor/task.h"

namespace bibblevm::executor {
    enum class SchedulerMessageType {
        Errored,
        Called,
        Returned,
        Yielded,
        Awaiting
    };

    struct SchedulerMessage {
        SchedulerMessageType type;
        union {
            struct { Function* function; uint16_t argsBegin; } call;
            Value returnValue;
            Future* future;
        };

        constexpr SchedulerMessage(SchedulerMessageType type) : type(type) {}

        static constexpr SchedulerMessage Errored() { return SchedulerMessageType::Errored; }
        static constexpr SchedulerMessage Called(Function* function, uint16_t argsBegin) {
            SchedulerMessage m = SchedulerMessageType::Called;
            m.call.function = function;
            m.call.argsBegin = argsBegin;
            return m;
        }
        static constexpr SchedulerMessage Returned(Value returnValue) {
            SchedulerMessage m = SchedulerMessageType::Returned;
            m.returnValue = returnValue;
            return m;
        }
        static constexpr SchedulerMessage Yielded() { return SchedulerMessageType::Yielded; }
        static constexpr SchedulerMessage Awaiting(Future* future) {
            SchedulerMessage m = SchedulerMessageType::Awaiting;
            m.future = future;
            return m;
        }
    };
}

#endif // BIBBLEVM_EXECUTOR_SCHEDULER_MESSAGE_H
